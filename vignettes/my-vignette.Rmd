...
TTB is currently only implemented for binary cues.

Here is some fake data on high school drop-out rates to consider.
```{r}
rural_schools <- data.frame(School=c('Allen', 'Baker', 'Cooper'), Dropout_Rate=c(40, 34, 30), Low_Income=c(1, 1, 0), Math=c(0, 1, 1), Reading=c(1, 0, 0))
rural_schools
```
...
ttb <- ttbBinModel(rural_schools, 2, c(3:5))
ttb$cue_validities
```
...
```{r}
ttb_predictions <- predict(ttb, rural_schools)
cueValidity(rural_schools$Dropout_Rate, ttb_predictions)
```

Not bad.

Now let us get data from another school district and see how well it predicts there.
```{r}
more_rural_schools <- data.frame(School=c('Franklin', 'Greer', 'Houser'), Dropout_Rate=c(41, 31, 36), Low_Income=c(1, 0, 0), Math=c(0, 0, 0), Reading=c(0, 1, 1))
more_rural_schools
ttb_predictions <- predict(ttb, more_rural_schools)
cueValidity(more_rural_schools$Dropout_Rate, ttb_predictions)
```
...
```{r}
# ttb_more_rural_schools <- ttbModel(more_rural_schools, 2, c(3:5))
ttb_more_rural_schools <- ttbBinModel(more_rural_schools, 2, c(3:5))
ttb_predictions <- predict(ttb_more_rural_schools, more_rural_schools)
cueValidity(more_rural_schools$Dropout_Rate, ttb_predictions)
```
...
```{r}
reg <- regModel(rural_schools, 2, c(3:5))
reg_predictions <- predict(reg, more_rural_schools)
cueValidity(more_rural_schools$Dropout_Rate, reg_predictions)
```
