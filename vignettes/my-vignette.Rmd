...
```{r}
rural_schools <- data.frame(School=c('Allen', 'Baker', 'Cooper'), Dropout_Rate=c(40, 34, 30), Low_Income=c(29, 25, 8), Math=c(83, 111, 110), Reading=c(99, 99, 98))
rural_schools
```
...
more_rural_schools
ttbPredictions <- predict(ttb, more_rural_schools)
cueValidity(more_rural_schools$Dropout_Rate, ttbPredictions)
```
...
ttbPredictions <- predict(ttb_more_rural_schools, more_rural_schools)
cueValidity(more_rural_schools$Dropout_Rate, ttbPredictions)
```
