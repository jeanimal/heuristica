...
```{r}
schools <- abc_hs_dropout[which(abc_hs_dropout$Name_of_School %in% c("Bowen", "Collins", "Young")),]
...
```{r}
pctCorrectOfPredictAlternative(list(ttb), schools)
```
...
```{r}
more_schools <- abc_hs_dropout[which(abc_hs_dropout$Name_of_School %in% c('Fenger', 'Juarez', 'Kenwood')),]
...
more_schools
pctCorrectOfPredictAlternative(list(ttb), more_schools)
```
...
```{r}
ttb_more_schools <- ttbBinModel(more_schools, 2, c(3:5))
pctCorrectOfPredictAlternative(list(ttb_more_schools), more_schools)
```
...
pctCorrectOfPredictAlternative(list(ttb, reg), schools)
```
...
```{r}
pctCorrectOfPredictAlternative(list(ttb, reg), more_schools)
```
Regression did better in this case.
