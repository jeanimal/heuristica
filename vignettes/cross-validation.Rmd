...
```{r}
crossV <- function(vec_of_models, criterion_col, cols_to_fit, x, train_size, reps) {
  results <- vector()
  for ( i in 1:reps) {
...
for (s in 1:length(trainS)){
  train_size <- trainS[s]
  accuracy <- rbind(accuracy, crossV(vec_of_models, criterion_col, cols_to_fit, city_population, train_size, reps))
}
```
