...
```{r}
vec_of_models <-c(ttbModel, dawesModel, franklinModel, regModel, logRegModel)
```
...
Franklin model and dawes model do really well at all training set sizes.  Take the Best is not far behind and catches up at about 40% of the training set size.  Regresssion and logistic regression are noticeably weaker, scoring barely above chance for small training sets, when there are more rows than the 23 columns.  They do better as the training set gets larger, and logistic regression is eventually able to beat regression by a bit, although it does not catch up to the heuristics.