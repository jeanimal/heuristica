...
```{r}
crossV <- function(vec_of_models, criterion_col, cols_to_fit, data, reps){
  fitting <- vector()
  prediction <- vector()
  for(i in 1:reps){
...
```{r}
reps <- 100
results <- crossV(vec_of_models, criterion_col, cols_to_fit, data_set, reps)
```
