...
crossV <- function(vec_of_models, criterion_col, cols_to_fit, x, train_size, reps, max_test_size) {
  ...
  for ( i in 1:reps) {
    ...
    # Keep test set a constant size of 8 for comparable statistics.
    test_trim_size <- min(max_test_size, nrow(test))
    test <- head(test, test_trim_size)
    #print(paste("iteration", i, " nrow(test):", length(test)))
...
for (s in 1:length(trainS)){
  train_size <- trainS[s]
  accuracy <- rbind(accuracy, crossV(vec_of_models, criterion_col, cols_to_fit, city_population, train_size, reps, max_test_size))
}
```
