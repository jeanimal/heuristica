...
predictAccuracy2 <- function(test_matrix, criterion_col, cols_to_fit, col_weights) {
  predictions <- predictWithWeightsLog(test_matrix, cols_to_fit,criterion_col, col_weights)
  return( logAccuracy(predictions) )
}
```
...
multiPredict75 <- function(vec_of_models, num_rand_columns) {
  ...
  for (model_constructor in vec_of_models) {
    ...
    if(class(model)[1] == "logRegModel"){
      predict_accuracy <- predictAccuracy2(m_test, criterion_col, cols_to_fit, model$linear_coef)
    } else {
      predict_accuracy <- predictAccuracy(m_test, criterion_col, cols_to_fit, coef(model))  
    }
...
```{r}
multiPredict75(c(dawesModel, franklinModel, ttbBinModel, regNoIModel, regModel, logRegModel), 0)
```
...
```{r}
model_list <- c(dawesModel, franklinModel, ttbBinModel, regNoIModel, regModel, logRegModel)
out <- data.frame()
