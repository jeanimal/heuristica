...
rowPairApply(oneRow(schools, 1), oneRow(schools, 2), correctGreater(2),
             heuristics(myFit, fn=predictPairInternal))
```

What if we want to see results for all pairs of the first 5 schools?  Use allRowPairApply.

```{r}
allRowPairApply(schools, correctGreater(2), heuristics(myFit, fn=predictPairInternal))
```
...
```{r}
set.seed(1)
predictions <- data.frame(allRowPairApply(schools, correctGreater(2), heuristics(myFit, fn=predictPairInternal)))
...
```{r}
out <- data.frame(allRowPairApply(my_data, rowIndexes(), heuristics(lasso,  fn=predictPairInternal), correctGreater(lasso$criterion_col)))
