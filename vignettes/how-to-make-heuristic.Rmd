...
comparisons with heuristica's heuristics.  These are a fitting function and two predicting functions.
...
Most statistical models in R implement "predict," which takes N rows of data and outputs N predictions.  Heuristica, however, is focused on predicting *pairs* of rows, so its models implement a "predict pair" and "predict pair probability".  Let us start with the former.
...
Now to use this function, we call predictPair and pass in the fitted model.  Let's get some data to make this clearer.
...
We can see in the original data.frame that Austin had the higher dropout rate, meaning 1 would have been correct output.  We can get heuristica to show this using a more general rowPair function to view the correct probability based on the criterion, which will be -1 or 1.
```{r}
myFit <- myRandModel(schools, 2, c(3:5))
rowPairApply(oneRow(schools, 1), oneRow(schools, 2), correctGreater(2),
             heuristics(myFit))
```

What if we want to see results for all pairs of the first 5 schools?  Use allRowPairApply.

```{r}
allRowPairApply(schools, correctGreater(2), heuristics(myFit))
```
...
Heuristica has two ways to assess the performance of this categorizing of ranking.  First, there is a confusion matrix, as in many machine learning problems, except this one assumes you always want to see the output for -1, 0, 1 always.  We need to give it the correct answers, which are based on column 2, so we use correctGreater(2).  And we need to give itht epredictions, which we generate with heuristics(myFit).  We pass these generators into allRowPairApply.  Then we send the output to our confusion matrix function.

```{r}
set.seed(1)
predictions <- data.frame(allRowPairApply(schools, correctGreater(2), heuristics(myFit)))
confusionMatrixPredictPair(predictions$CorrectGreater, predictions$myRandModel)
```
...
What percent correct is this?  In this case, it's an easy division of 6/10 = 0.6.  But you can get this result in one step-- for many fitted models-- with pctCorrectOfPredictPair.  Below is the call (with the random seed set again).
...
#### PredictProbInternal and PredictProb

Note that instead of the predicting which row is greater, we could predict the probabliity that row1 is greater.  Its output should be a value from 0 to 1.  When a consistent function predicts that the probability that row 1 is greater is more than 0.5 (#2), it should also select row1 as greater (#1).  Here is an example implementation for myRandModel.

```{r}
predictProbInternal.myRandModel <- function(object, row1, row2) {
  prob <- runif(1)
  return(prob)
}
```
...
The task is selecting between two rows.  so lasso should predict each row and choose the one with the higher criterion.  Below is an example implementation or predictProbInternal that will work, although it is not very efficient.  Since there is no probability calculated, we simply return 0, 1, or 0.5 for the probability.  In contrast, logistic regression explicitly calculates a probability.
...
Which comparison did it miss?  Check out the helper function below.  We predict all row pairs and then find the row pairs where Lasso did not agree with the Correct answer.  Lasso missed only one comparison, namely row 3 vs. row 4, where it guessed.  It got the other 5 row pairs correctly!
```{r}
out <- data.frame(allRowPairApply(my_data, rowIndexes(), heuristicsProb(lasso,  fn=predictPairInternal), correctGreater(lasso$criterion_col)))
