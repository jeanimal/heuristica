...
For our bare-bones example, we will just always return 1, meaning we predict row1 always has a greater criterion than row2.  This is dumb, because if our model is asked about a vs. b, it will say a is bigger, and if it is asked about b vs. a, it will say b is bigger.  It decides simpyl based on which came first.
...
```
  ...
  We "fit" myModel to the schools data.  Then we extract two rows to compare, which happen to be Austin and Farrgut.  Finally we ask the model to predict the probability that Austin has the higher drop-out rate, and of course it answers 1, Austin is higher, because that's how we hard-coded it.
...
However, if we had asked about Farrgaut vs. Austin, it says Farrgut, which is wrong.
```{r}
myFit <- myModel(schools, 2, c(3:5))
rowPairApply(oneRow(schools, 2), oneRow(schools, 1), criterion(2), heuristics(myFit))
```
Across these two cases, it got one right and one wrong, for 50% correct.

Heuristica makes this sort of assessment easy-- it cn check your model agains all pairs and calculate the percent correct.  Here we do myModel for all pairs of schools in our subset data.  Not surprisingly, it earns 50% correct.
```{r}
myFit <- myModel(schools, 2, c(3:5))
pctCorrectOfPredictPair(list(myFit), schools)
```

Oops that gave us the wrong answer because myModel is not symmetric.  So it's a bad example.  But below you can see I can generate the right answer:
```{r}
myFit <- myModel(schools, 2, c(3:5))
pctCorrectOfPredictPairNonSymmetric(list(myFit), schools)
```
