...
Now to use this function, we call predictPair and pass in the fitted model.  We could call predictPairInternal directl, but predictPair takes care of some housekeeping.  (It calls `predictPairInternal(object, row1[cols_to_fit], row2[cols_to_fit])` and makes sure one row is returned and names its column with the model class or `fit_name`.)  Let's get some data and run our model on it.
