...
```{r}
predictPairInternal.myModel <- function(object, row1, row2) {
  return(sample(c(-1,1), 1))
}
```

TODO(jean): Give an example with predictProbInternal.myModel and explain the difference.
...
TODO(jean): Explain the difference.
```{r}
...
```{r}
allRowPairApply(my_data, rowIndexes(), heuristics(lasso,  fn=predictPairInternal), correctGreater(lasso$criterion_col))
```
