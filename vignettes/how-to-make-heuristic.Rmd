...
The task is selecting between two rows.  so lasso should predict each row and choose the one with the higher criterion.  Below is an example implementation or predictRoot that will work, although it is not very efficient.
```{r}
predictRoot.lassoModel <- function(object, row1, row2) {
...
```{r}
predictRowPair(oneRow(my_data, 1), oneRow(my_data, 2), lasso)
```
...
```{r}
allRowPairApply(my_data, rowIndexes(), heuristics(lasso), criterion(lasso$criterion_col))
```
