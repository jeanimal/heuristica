...
```{r}
myFit <- myRandModel(schools, 2, c(3:5))
# TODO(jean) Create a wrapper where default is predictPairInternal.
rowPairApply(oneRow(schools, 1), oneRow(schools, 2), correctGreater(2),
             heuristicsProb(myFit, fn=predictPairInternal))
```

What if we want to see results for all pairs of the first 5 schools?  Use allRowPairApply.

```{r}
allRowPairApply(schools, correctGreater(2), heuristicsProb(myFit, fn=predictPairInternal))
```
...
Heuristica has two ways to assess the performance of this categorizing of ranking.  First, there is a confusion matrix, as in many machine learning problems, except this one assumes you always want to see the output for -1, 0, 1 always.

```{r}
set.seed(1)
predictions <- data.frame(allRowPairApply(schools, correctGreater(2), heuristicsProb(myFit, fn=predictPairInternal)))
