...
A confusion matrix compares the predicted values with the actual, correct values.  rowPairApply can generate these values, and here we walk through an example
...
Generate data using rowPairApply.  Since we request correctGreater, then ttb, then reg, the output column names will be "CorrectGreater", "ttbModel", and "regModel".
...
Take a look at a few rows.  But these are boring rows.
```{r}
head(out)
```
